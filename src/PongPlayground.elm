module PongPlayground exposing
...
spin frequency time =
    if frequency == 0 then
        ...
        360 * toFrac (1 / frequency) time



-- pourquoi la période plutôt que la fréquence?
...
wave lo hi frequency time =
    if frequency == 0 then
        0

    else
        lo + (hi - lo) * (1 + cos (turns (toFrac (1 / frequency) time))) / 2
...
zigzag lo hi frequency time =
    if frequency == 0 then
        0
    else 
    lo + (hi - lo) * abs (2 * toFrac (1 / frequency) time - 1)
